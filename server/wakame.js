...
async function getapis() {
    try {
        const response = await axios.get('https://raw.githubusercontent.com/wakame02/wktopu/main/inv.json');
    ...
    for (const instance of apis) {
        try {
            ...
            if (response.data && (response.data.formatStreams || response.data.adaptiveFormats)) {
                ...
                console.error(`formatStreams or adaptiveFormats が存在しない: ${instance}`);
            }
        } catch (error) {
...
async function getYouTube(videoId) {
    try {
        ...
        if (videoInfo.hlsUrl) {
            streamUrl = videoInfo.hlsUrl; // hlsUrlを直接使う
        }
